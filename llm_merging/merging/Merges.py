...
class Merges(object):
    ...
    def _load_base_model(self):
        bnb_config = BitsAndBytesConfig(load_in_8bit=True, 
                                       ...
                                       llm_int8_skip_modules=["lm_head"]) #, bnb_4bit_use_double_quant=True,
                                                    # bnb_4bit_quant_type="nf4",
                                                     #bnb_4bit_compute_dtype=torch.bfloat16,
                                                     #)
        if self.architecture == "encoder_decoder":
            self.base_model =  AutoModelForSeq2SeqLM.from_pretrained(self.base_model_name, revision=self.base_model_revision_id, quantization_config=bnb_config, token=os.environ["HF_AUTH_TOKEN"], device_map='auto')#.to(self.device)
        elif self.architecture == "decoder":
            self.base_model =  AutoModelForCausalLM.from_pretrained(self.base_model_name, revision=self.base_model_revision_id, quantization_config=bnb_config, token=os.environ["HF_AUTH_TOKEN"], device_map='auto')#.to(self.device)
        else:
